...
################################################################################
# Functions you will want to use:
# - probably just get_temperature_adjusted_ranks.
#   - just give it a matrix and you shall receive... the temperature adjusted
#   ranks
# - get_springranks (pass it an adjacency matrix)
# - get_temperature (pass it the adjacency matrix you gave to get_springranks
# and the ranks you get from springrank)
################################################################################
def get_temperature_adjusted_ranks(matrix):
    ranks = get_springranks(matrix)
    temperature = get_temperature(matrix, ranks)
    return ranks * temperature


def get_springranks(matrix):
    """given an adjacency matrix, returns springrank's ranking of that matrix.
    the minimum rank will be 0."""
    ranks = SpringRank(matrix, alpha=0, l0=1., l1=1.)

    # shift the ranks so they start at 0
    min_rank = min(ranks)
    ranks = np.array([rank - min_rank for rank in ranks])
    return ranks


def get_temperature(matrix, ranks):
    """given an adjacency matrix and the ranks for that matrix, calculates the
    temperature of those ranks"""
    betahat = brentq(eqs39, 0.01, 20, args=(ranks, matrix))
    temperature = 1 / (np.log(0.8/0.2) / (2 * betahat))
    return temperature
...
################################################################################
# SpringRank Planted Network stuff
################################################################################
